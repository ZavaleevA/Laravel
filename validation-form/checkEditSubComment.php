<?php
	$idEditSubComment = $_GET['idEditSubComment'];
	...
	$result = $mysql->query("SELECT * FROM `reply_comment` WHERE `id_reply`='$idEditSubComment'");
    while( $row = mysqli_fetch_assoc($result) ) {
        $comment_user = $row['reply_comment'];    
    }
	if ($comment_user != $comment) {
		$mysql->query("UPDATE `reply_comment` SET `reply_comment` = '$comment', `edit_date_reply` = '$date' WHERE `id_reply`= '$idEditSubComment'");
	}   
	$mysql->close();
