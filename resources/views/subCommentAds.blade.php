...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-11">
            @if (session('errorSubComment'))
                <div class="alert alert-danger">
                    {{session('errorSubComment')}}
                </div>
            @endif
            @if (session('successSubComment'))
                    <div class="alert alert-success">
                    {{session('successSubComment')}}
                    </div>
                @endif
            @foreach($dataCommentAds as $comment)
                <div class="alert alert-info">
                    <h2>{{ ('Вы отвечаете на этот комментарий:') }}</h2>
                    <table border="0" width="100%" cellpadding="7">
                        <td rowspan="2" width="17%">
                        @if((($userAvatar = DB::table('users')->where('id', $comment->user_id)->value('avatar')) != NULL) AND $noComments = 1)
                        ...
                        <td align='center' style="color: Black;"><strong>{{DB::table('users')->where('id', $comment->user_id)->value('name')}}</strong></td>
                        <td height="20%" colspan="2" bgcolor='white' align='right'>
                        <a style="color: Black;" class="btn btn-warning" href="{{route('date-ads', $comment->ads_id)}}">Отмена</a></td><tr>
                        </table>
                </div>
            @endforeach
        </div>    
        <div class="col-md-8">    
            <div class="card">
                <div class="card-body"  align="center">
                    ...
                    <form action="{{ route('add-sub-comment-ads', $comment->id) }}" method="POST">
                        @csrf
                        <textarea class="form-control" name="newSubComment" id="newSubComment" rows="3" placeholder="Ваш ответ накомментарий"></textarea><br>
                        <button class="btn btn-warning" type="submit">Ответить</button>
                        <a href="{{route('date-ads', $comment->ads_id)}}" class="btn btn-info" >Назад</a>
                    </form>  
                </div>
            </div>
        </div>
